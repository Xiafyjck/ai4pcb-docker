...
```text
/opt/boxctl/README.md
/opt/boxctl/vpn/README.md
/opt/boxctl/fsferry/README.md
```

刻意放 `/opt` 而不是 `/root`：文档跟代码同属一个镜像层，版本永远对得上。`/root` 是你的
家目录、平台又靠快照留着它，塞进去的文档会在容器里冻住，镜像更新了也不跟着变。`/root`
下只有 `.boxctl.env` 一个文件，因为那本来就是你要改、要保留的东西。

什么都没配之前，交互式登录会提示一行「还没启用任何服务」，配好就自然消失。它只在 stderr
是终端时打——平台启动脚本 source `.bashrc` 时没有 tty，不会污染它的日志。
